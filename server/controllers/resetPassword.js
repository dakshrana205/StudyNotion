...
exports.resetPasswordToken = async (req, res) => {
    try {
        ...
        console.log('Password reset URL:', url); // Debugging

        await mailSender(email, 'Password Reset Link', `Password Reset Link : ${url}`);

        res.status(200).json({
            success: true,
            message: 'Email sent successfully. Please check your mailbox and change the password.'
        });
    } catch (error) {
        console.log('Error while creating token for reset password:', error);
        res.status(500).json({
            success: false,
            error: error.message,
